...
class AssemblyQciController extends GetxController {
  ...
  // ── SA Final saved-form list state ─────────────────────────────────────────
  ...
  Future<void> captureQciVisualFailureImage({
    ...
    final File? image = await PickImageService.captureAndSaveImage(
  ...
  Future<void> setQciShiftFromDb() async {
    ...
    for (final item in shifts) {
      shiftList.add(
        SelectOptions(key: item.id, value: item.name, code: item.code),
      );
    }
  ...
  Future<void> setQciShellNoFromDb() async {
    ...
    selectedShellNo = shellNoList.firstWhereOrNull(
      (e) => e.key == previousKey && e.key != '0',
    );
    update();
  }
  ...
  // FIX 4 — save params to Hive (was commented out before)
  Future<void> getSaQciFormParams({
    ...
    await GetApiProvider().onGetProviderWithParam(
      ...
      onSuccess: (result) async {
        try {
          final rawMap = Map<String, dynamic>.from(result.toJson());

          // ✅ FIX 4: actually save to Hive
          await _qciFormParamService.savePayload(
            payload: {'frame_type_id': frameTypeId, 'data': rawMap},
          );

          if (kDebugMode) {
            debugPrint('[SA QCI] form params saved for $frameTypeId');
          }
          await onComplete(true);
        } catch (e) {
          if (kDebugMode) debugPrint('[SA QCI] getSaQciFormParams error: $e');
          await onComplete(false);
        }
      },
      onError: (_) async {
        if (kDebugMode) debugPrint('[SA QCI] getSaQciFormParams API error — using cache');
        await onComplete(false);
      },
    );
  }

  /// Pre-fetches QCI form params for every known frame type and saves to Hive.
  Future<void> prefetchAllQciFormParams() async {
    final frameTypes = allFrameTypesService.getTypesByTrainType('lhb');
    for (final ft in frameTypes) {
      await getSaQciFormParams(
        frameTypeId: ft.id,
        onComplete:  (_) async {},
      );
    }
  ...
  // ── Form loading from Hive / asset ──────────────────────────────────────────

  // FIX 3 — cached is Map, not String — no jsonDecode needed
  Future<void> loadQciFormFromHive({required String frameTypeId}) async {
    final cached = _qciFormParamService.getFormDataByFormId(frameTypeId);
    if (cached != null && cached.isNotEmpty) {
      try {
        // ✅ FIX 3: cached is already Map<String, dynamic>
        qciForm = SaQciFormUiModel.fromJson(cached);
        update();
        return;
      } catch (_) {}
    }
  ...
  // FIX 5 — removed hardcoded if(true)
  ...
  // FIX 1 + 2 — moved _resetQciForm outside, added actual Hive save
  Future<void> _saveQciInspection() async {
    ...
    final formData = <String, dynamic>{
      ...
      'visual_obs':                   Map<String, String?>.from(_selectedObservedOptions),
    ...
    _resetQciForm(); // ✅ FIX 1: called here, defined below as separate method
  }

  // ✅ FIX 1 — _resetQciForm is now a proper class method
  void _resetQciForm() {
    _setDefaults();
  ...
  Future<void> _trySyncQciPayloads() async {
    final unsynced = _qciFormPayloadService.getUnsyncedPayloads();
    if (unsynced.isEmpty) return;

    for (final entry in unsynced.entries) {
      try {
        await GetApiProvider().onPostProvider(
          url:  ConfigApi.submitSaQciForm,
          data: entry.value,
          onSuccess: (_) async {
            await _qciFormPayloadService.updateSyncStatus(
              formId:   entry.key,
              formSync: 1,
            );
          },
  ...
  int get saFinalUnsyncedPayloadCount =>
      _qciFormPayloadService.getUnsyncedPayloads().length;

  bool get hasSaFinalOfflinePendingSync => saFinalUnsyncedPayloadCount > 0;

  bool get shouldShowSaFinalApiList =>
      !hasSaFinalOfflinePendingSync && saFinalUnsyncedPayloadCount == 0;

  List<dynamic> getSaFinalOfflineSavedRows() =>
      _qciFormPayloadService.getUnsyncedPayloads().values.toList();

  // ── SA Final saved list (API pagination) ────────────────────────────────────
